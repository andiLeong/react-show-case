...
function App() {
    ...
    return (
        <div className={`grid gap-1 grid-cols-12 `}>
            <div className={`space-y-2 col-span-4 grid grid-cols-4 gap-1 content-start`}>
                <Create todos={todos} setTodos={setTodos}/>
            </div>

            {
                todos.length > 0 ? (
                    <div style={{height: '27em'}} className={`space-y-8 divide-y col-span-5  flex flex-col`}>
                        <div className={`flex-1`}>
                            <Lists mapAndSetTodo={mapAndSetTodo} setTodos={setTodos} todos={todos}
                        ...
                        <div className={`flex items-center justify-between pt-4`}>
                            <div className={`flex`}>
                                <Filters filterCounts={filterCount()} filter={filter} setFilter={setFilter}/>
                            </div>
